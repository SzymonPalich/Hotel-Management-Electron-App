    ...
    <div class="mt-4 flex mr-0 ml-auto">
      ...
      <div class="pr-6 flex items-center">
        <img
          ...
          @click="$router.push({ name: 'room_types-create' })"
        />
      </div>
    </div>
      ...
      <div class="overflow-auto rounded-xl">
        <table class="min-w-full">
          ...
          <tbody class="text-gray-700">
            <tr
              v-for="room_type in result.content"
              :key="room_type"
              class="bg-white"
            >
              ...
              <td class="text-center py-2 px-4 w-36">
                ...
                <img
                  @click="alertDisplay(room_type.id)"
                  class="material-icons align-middle"
...
export default defineComponent({
  ...
  data() {
    return {
      result: Utils.getBlankListTemplate<IRoomType>(),
  ...
  methods: {
    async getData(): Promise<IList<IRoomType>> {
      return await RoomTypesService.getList(this.pager);
    },

    async find(): Promise<void> {
      this.result.pager.search = this.search;
      this.result = await RoomTypesService.getList(this.result.pager);
    },
